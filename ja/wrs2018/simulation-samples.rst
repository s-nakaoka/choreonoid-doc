...
なお、T2のサンプルではまだターゲットマーカーは配置されていません。マーカーのモデルは :ref:`wrs_standard_model_directory` のWRS2018以下に "Target1.body" というファイル名で格納されていますので、練習にはこちらのモデルを用いてください。（ターゲットマーカーはサイズについていくつかのバリエーションを用意することになっており、順次モデルを追加していきます。）
...
一見T2と同様の構成になっていますが、車内には要救助者（ダミーターゲット）が閉じ込められています。実際の競技では、ドアは通常の操作では開かなくなっています。油圧スプレッダがトンネル内に置いてありますので、それを操作してドアのロックやヒンジの部分に押し当てることで、ドアを外すことができます。この部分は近々サンプルに導入します。ドアを外したら、邪魔にならないように移動させた上で、ダミーターゲットを車内より搬出するという作業になります。
...
サンプルでは、トンネル内に障害物がいくつか散らばっているので、それをロボットで押したり持ち上げたりして、これらの障害物を除去する操作を行ってください。なお、現状のサンプルは障害物の配置の仕方などが実際の競技とは少し異なる内容となっています。正確な競技内容は、についてはやはり :ref:`wrs2018_overview_rules` で確認してください。また、追加の障害物として、タスク開始点に動かなくなった車両が配置される予定です。これについても順次サンプルに導入してまいります。
...
ホースの先端をつかんで引っ張ると、ホースを引き出すことができます。まずはこの操作を試してみてください。

タスクとしては、ホースを引き出し、先端をノズルと接続し、バルブを回し、ノズルのレバーを引くと、ノズルから水が出ます。この部分のシミュレーションの対応については今しばらくお待ちください。完成次第アップします。
