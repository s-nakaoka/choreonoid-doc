...
本節で紹介した干渉検出機能は、 :doc:`../simulation/index` とは基本的に独立して行われるものとなっています。現在の仕様では、WorldアイテムやBodyアイテムの「干渉検出」プロパティがtrueであってもfalseであっても、シミュレーションの挙動に変化はありません。シミュレーションにおける干渉検出は、シミュレータ内部の物理計算用モデルに対して行われるものであり、その処理内容は :ref:`simulation_simulator_item` によって決定されます。
