...
以上の例では、関節角度と関節トルクのみを入出力の対象としました。しかし、ロボットには他にも入出力の対象となるものがあります。Choreonoidでは、関節以外でロボットの入出力の対象となるものを、「デバイス」として定義しています。デバイスの例として、具体的には

* 力センサ、加速度センサ、角速度センサ（ジャイロ）
* カメラ、レーザーレンジファインダ

といったセンサ類がまず挙げられます。これらは主に入力の対象となりますが、カメラのズーム変更等、操作指令を出力したい場合もあります。

また、主に出力の対象となるものとして、以下のようなデバイスもあり得ます。

* ライト（光源）
* スピーカ（音源）

実用的なコントローラを開発するためには、デバイスに対しても入出力を行えるようになっている必要があります。ただし、その方法もまたコントローラの形式によって異なってきますので、デバイスを使用する際にはコントローラの形式に従って入出力を行うようにしてください。

デバイスオブジェクト
--------------------

SimpleControllerでは、デバイスに対してもBodyアイテムを介して入出力を行います。正確に言うと、Bodyアイテムから得られる「Deviceオブジェクト」を用いて、対象となるデバイスへの入出力を行います。
