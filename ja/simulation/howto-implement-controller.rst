...
Bodyオブジェクトにおいて、上記の要素はロボットのリンクをモデル化した「Linkクラス」のオブジェクトに格納されます。各関節に対応するLinkオブジェクトは、Bodyオブジェクトの以下の関数を用いて取得できます。
...
これを行うためには、まずioオブジェクトに対して以下の関数による設定を行います。

* **void setLinkInput(Link\* link, int stateTypes)**

 コントローラへの入力となるリンク状態値を指定します。

* **void setLinktOutput(Link\* link, int stateTypes)**

 コントローラから出力されるリンク状態値を指定します。

上記関数のlink引数には対象となるLinkオブジェクトを指定します。引数stateTypesについては、今のところリンクの位置姿勢に対応する **LINK_POSITION** のみを指定できます。
