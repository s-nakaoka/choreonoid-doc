...
.. note:: ファイル名の設定において、拡張子は省略することが可能です。ファイル選択用ダイアログから選択した場合は、上記のように拡張子が省略されたかたちで入力されます。拡張子を省略した場合は、稼働しているOSに合わせた拡張子が内部で自動的に付与されます。従って、拡張子を省略することでどのOSでも利用可能なプロジェクトとすることができます。

.. note:: 実際のファイルは、シンプルコントローラアイテムの「ベースディレクトリ」プロパティで指定したディレクトリ内に格納されているものとなります。プロパティとして「コントローラディレクトリ」と「プロジェクトディレクトリ」が指定可能です。前者はシンプルコントローラの標準ディレクトリ、後者は現在読み込んでいるプロジェクトファイルが格納されているディレクトリとなります。またフルパスで指定する場合、ベースディレクトリは「なし」となります。
