...
今回は、コントローラによる制御対象となり得るモデルを用意します。 :ref:`bodymodel_samplemodels` の中に "SR1" というロボットモデルがありますので、これを使うことにしましょう。shareディレクトリの "model/SR1/SR1.yaml" を読み込んでください。
...
:ref:`simulation-time-step` も忘れずに行っておきましょう。今回もとりあえず 1000 [fps] としておくことにします。
...
* **シンプルコントローラアイテム**
 ...
 独自のコントローラ実装形式である「シンプルコントローラ」形式に対応したコントローラアイテムです。この形式は主にサンプルの実装を目的として、コントローラ実装のシンプルさを重視して設計されたものです。ただし汎用性はあまり重視されておらず、現実のロボットシステムに適用することを想定したものではありません。デフォルトで導入される「SimpleControllerプラグイン」によって利用可能となります。
